...
**Если macOS пишет «приложение повреждено и не может быть открыто»** — это Gatekeeper-карантин на скачанных файлах. Запусти в терминале:
```bash
xattr -cr "/Applications/PDF Presenter.app"
```
После этого приложение откроется. Это не повреждение, а строгая защита macOS от приложений без подписи Apple Developer ID.
